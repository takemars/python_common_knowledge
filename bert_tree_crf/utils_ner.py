...
class Processor(DataProcessor):
    ...
    def __init__(self, logger, dataset, latent_size):
        ...
        if dataset == "ACE04" or dataset == "ACE05":
            ...
            import json
            with open('./args2id.json', 'r', encoding='utf-8') as f:
                for l in f:
                    line = json.loads(l)
            self.labels = list(line.keys())
        else:
            raise NotImplementedError()
    ...
    def _create_examples(self, lines, type):
        ...
        for i in range(0, len(lines), self.interval):
            text_a = lines[i]
            label = lines[i + 1]
            # self.logger.info(f"lines[{i}]={text_a}")
    ...
    def convert_examples_to_features(self, examples, max_seq_length, tokenizer):
        ...
        for (ex_index, example) in enumerate(tqdm(examples)):
            ...
            fea = InputFeatures(input_ids=input_ids.copy(),
                              input_mask=input_mask.copy(),
                              segment_ids=segment_ids.copy(),
                              partial_masks=partial_masks.copy(),
                              gather_ids=gather_ids.copy(),
                              gather_masks=gather_masks.copy())
            # import copy
            # fea_copy = copy.deepcopy(fea)
            # fea = fea.copy()
            features.append(fea)
            # import gc
            # gc.collect()
    ...
    def convert_examples_to_features1(self, examples, max_seq_length, tokenizer):
        """Loads a data file into a list of `InputBatch`s."""
        tokens = tokenizer.tokenize(examples.text_a)
        gather_ids = list()
        for (idx, token) in enumerate(tokens):
            if (not token.startswith("##") and idx < max_seq_length - 2):
                gather_ids.append(idx + 1)
        ...
        partial_masks = self.generate_partial_masks(examples.text_a.split(' '), max_seq_length, examples.label,
                                                         self.labels)

        # self.logger.info("*** Example ***")
        # self.logger.info(f"text_a={examples.text_a}")
        # self.logger.info("guid: %s" % (examples.guid))
        # self.logger.info("tokens: %s" % " ".join(
        #     [str(x) for x in tokens]))
        # self.logger.info("input_ids: %s" % " ".join([str(x) for x in input_ids]))
        # self.logger.info("input_mask: %s" % " ".join([str(x) for x in input_mask]))
...
def eval(args, outputs, partial_masks, label_size, gather_masks, logger):
