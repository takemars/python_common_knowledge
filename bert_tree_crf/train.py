...
def train(args,  model, tokenizer, processor):
    ...
    # 实例化对象
    train_data = DueeDataset(processor, tokenizer, args.train_file, args)
    # 将数据集导入DataLoader，进行shuffle以及选取batch_size
    train_dataloader = DataLoader(train_data.train_examples, batch_size=args.per_gpu_train_batch_size, shuffle=True, num_workers=0)

    dev_data = DueeDataset(processor, tokenizer, args.predict_file, args)
    # 将数据集导入DataLoader，进行shuffle以及选取batch_size
    dev_dataloader = DataLoader(dev_data.train_examples, batch_size=args.per_gpu_eval_batch_size, shuffle=True, num_workers=0)

    test_data = DueeDataset(processor, tokenizer, args.test_file, args)
    # 将数据集导入DataLoader，进行shuffle以及选取batch_size
    test_dataloader = DataLoader(test_data.train_examples, batch_size=args.per_gpu_eval_batch_size, shuffle=True, num_workers=0)
    ...
    for _ in train_iterator:
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        results = evaluate(args, model, dev_dataloader, tokenizer, processor, global_step, _, "dev")
        ...
        evaluate(args, model, dev_dataloader, tokenizer, processor, global_step, _, "dev")
        evaluate(args, model, test_dataloader, tokenizer, processor, global_step, _, "test")
...
def evaluate(args, model, eval_dataloader, tokenizer, processor, step, _, prefix):
    global best_F
...
class DueeDataset(Dataset):
    def __init__(self, processor, tokenizer, file_path, args):
        # 创建一个list用来储存图片和标签信息
        # 打开第一步创建的txt文件，按行读取，将结果以元组方式保存在imgs里
        logger.info(f"filepath===={file_path}")
        self.train_examples = processor.get_train_examples(input_file=file_path)
    ...
    def __getitem__(self, index):
        train_feature = self.processor.convert_examples_to_features1(self.train_examples[index], args.max_seq_length,
                                                                self.tokenizer)
        return self.getTensorDataset(train_feature)
        # return train_dataset

    def getTensorDataset(self, f):
        ...
        all_gather_ids = torch.tensor(f.gather_ids, dtype=torch.long)
        all_gather_masks = torch.tensor(f.gather_masks, dtype=torch.long)
        all_partial_masks = torch.tensor(f.partial_masks, dtype=torch.long)
...
def main():
    ...
    if args.do_train:
        global_step, tr_loss = train(args,  model, tokenizer, processor)
